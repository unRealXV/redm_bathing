...
AddEventHandler('redm-bathing:client:StartBath', function(town)
    ...
    if Config.BathingZones[town] then
        ...
        Citizen.InvokeNative(0x4820A6939D7CEF28, PlayerPedId(), true)
        HolsterPedWeapons(PlayerPedId(), false, false, false, true)
        ...
        SetAnimSceneEntity(currentAnimScene, "ARTHUR", PlayerPedId(), 0)
        ...
        while DoesCamExist(currentCam) do
            while not IsTaskMoveNetworkReadyForTransition(PlayerPedId()) do Wait(100) end
            ...
            if IsControlPressed(0, `INPUT_CONTEXT_X`) and IsPromptEnabled("SCRUB") then
                ...
                while not IsTaskMoveNetworkReadyForTransition(PlayerPedId()) do Wait(10) end
...
ExitBathing = function()
    ...
    SetAnimSceneEntity(outroScene, "ARTHUR", PlayerPedId(), 0)
    ...
    TriggerServerEvent("redm-bathing:server:setCleanliness", 100)
    TriggerEvent('hud:client:UpdateCleanliness', 100)
    TriggerServerEvent("redm-bathing:server:setBathAsFree", town)
...
AddEventHandler('redm-bathing:client:StartDeluxeBath', function(town)
    ...
    local model = IsPedMale(PlayerPedId()) and Config.BathingZones[town].lady or Config.BathingZones[town].guy
    LoadModel(model)
    BathingPed = CreatePed(model, GetEntityCoords(PlayerPedId())-vector3(0.0, 0.0, -5.0), 0.0, false, false, true, true)
...
function UndressCharacter()
    ...
    Citizen.InvokeNative(0x4820A6939D7CEF28, ped, false)

    -- Fallback for clothing scripts (optional)
    -- TriggerEvent('jo_clothingstore:removeAllClothes')

    -- Official RDR2 Category Hashes
    local allCategories = {
        0x18729F39, 0x1D4C528A, 0x3C1A74CD, 0x9925C067, 0x485EE834,
        0x3107499B, 0x2026C46D, 0x3F7F3587, 0x7505EF42, 0x514ADCEA,
        0x5FC29285, 0x777EC6EF, 0x79D7DF96, 0x7A6BBD0B, 0xF16A1D23,
        0x7BC10759, 0x91CE9B20, 0x7A96FACA, 0x7BE77792, 0x7C00A8F0,
        0x83887E88, 0x877A2CF7, 0x8EFB276A, 0x94504D26, 0x9B2C8B89,
        0xA6D134C6, 0xFAE9107F, 0xB6B6122D, 0xB9E2FA01, 0xDA0E2C55,
        0xDB64A390, 0xE06D30CE, 0x0662AC34, 0xAF14310B, 0x72E6EF74,
        0x05E47CA6, 0xEABE0032, 0x1AECF7DC, 0x3F1F01E5, 0x49C89D9B,
        0x76F0E272, 0xA0E3AB7F, 0x4A73515C, 0xF1542D11, 0x8E84A2AA,
        0xA2926F9B
    }

    for _, hash in ipairs(allCategories) do
        -- Brute-force removal: Execute both stripping natives without checking if it's worn
        -- 0xD710A5007C2AC539 = _REMOVE_TAG_FROM_META_PED
        Citizen.InvokeNative(0xD710A5007C2AC539, ped, hash, 0)
        
        -- 0xDF631E4BCE1B1FC4 = _REMOVE_ITEM_FROM_PED_BY_CATEGORY
        Citizen.InvokeNative(0xDF631E4BCE1B1FC4, ped, hash, true, true, true)
        
        -- Skirt fallback to prevent invisible lower body
        if not isMale and hash == 0xA0E3AB7F then
            Citizen.InvokeNative(0xD3A7B003ED343FD9, ped, 4726031, false, true, true) 
...
DressCharacter = function()
    ...
    -- Break animation first
    ClearPedTasksImmediately(ped)
    Wait(500) 

    -- Fallback for your clothing script to change here
    --TriggerEvent('jo_clothingstore:resetClothes')
    
    ExecuteCommand('rc') -- if you want to use your clothingscript, repalce to --ExecuteCommand('rc')
    
    -- Await model render
    Wait(1500) 
    
    local maxStamina = Citizen.InvokeNative(0xCB42AFE2B613EE55, ped, Citizen.ResultAsFloat())
    local currentStamina = Citizen.InvokeNative(0x775A1CA7893AA8B5, ped, Citizen.ResultAsFloat()) / maxStamina * 100

    -- Reset dirt
    Citizen.InvokeNative(0x9C720776DAA43E7E, ped, 0.0)
    Citizen.InvokeNative(0x44CB6447D2571AA0, ped, -1.0)
    
    -- Show weapons again
    Citizen.InvokeNative(0x4820A6939D7CEF28, ped, true)
    
    -- Restore stats
    SetEntityHealth(ped, currentHealth)
    Citizen.InvokeNative(0xC3D4B754C0E86B9E, ped, currentStamina)
end
