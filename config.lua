Locale = 'en' -- change this to switch language, e.g. 'de', 'fr', 'pt-br'

local function LoadLocaleFile(lang)
    local raw = LoadResourceFile(GetCurrentResourceName(), ('locales/%s.lua'):format(lang))
    if not raw then return nil end
    local chunk = load(raw)
    return chunk and chunk() or nil
end

local Locales = LoadLocaleFile(Locale) or {}
local FallbackLocales = LoadLocaleFile('en') or {}

function locale(key, ...)
    local str = Locales[key] or FallbackLocales[key] or key
    if select('#', ...) > 0 then
        return str:format(...)
    end
    return str
...
Config.BathingZones = {
    ...
    -- Add further towns by copying one of the blocks above (dict, rag, consumer,
    -- lady/guy ped models, door hash) and giving it a unique key here.
}
